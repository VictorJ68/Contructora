...
public class ClientesController {
	...
	public List<Clientes> getAll(){
		return clientesServ.listarTodos();
	}
	...
	public Optional<Clientes> buscarPorId(@PathVariable Integer id){
		return clientesServ.buscarPorId(id);
	}
	...
	@ResponseBody
	public ResponseEntity<?> resgistra(@RequestBody Clientes obj) {
		HashMap<String, Object> salida = new HashMap<>();
		
		try {
			Clientes objSalida = clientesServ.registraClientes(obj);
			if (objSalida == null) {
	...
	@ResponseBody
	public ResponseEntity<Map<String, Object>> actualizar(@RequestBody Clientes obj){
		Map<String, Object> salida = new HashMap<>();
		
		try {
			Clientes objSalida = clientesServ.actualizaClientes(obj);
			if (objSalida == null) {
				salida.put("mensaje", AppSettings.MENSAJE_ACT_ERROR);
			} else {
	...
	public ResponseEntity<Map<String, Object>> eliminar(@PathVariable("id") int id){
		...
		try {
			clientesServ.eliminaClientes(id);
			salida.put("mensaje", AppSettings.MENSAJE_ACT_EXITOSO);
		} catch (Exception e) {
			e.printStackTrace();
			salida.put("mensaje", AppSettings.MENSAJE_ACT_ERROR);
	...
	@GetMapping("/search")
	@ResponseBody
	public List<Clientes> Buscar(@ModelAttribute("buscar") Clientes clientes) {
		ExampleMatcher matcher = ExampleMatcher.matchingAny() // matchingAny = OR
			    .withMatcher("nombre", ExampleMatcher.GenericPropertyMatchers.contains().ignoreCase())
			    .withIgnorePaths("id","direccion","telefono","correo","contacto");
		
		Example<Clientes> example = Example.of(clientes, matcher);
		
		return clientesServ.buscarByExample(example);
	}
