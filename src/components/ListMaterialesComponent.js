...
const ListMaterialesComponent = () => {
    ...
    const listarMateriales = () => {
        MaterialService.getAllMateriales()
            .then(response => {
    ...
    const handleSearchChange = (text) => {
        ...
        // Si tienes endpoint de búsqueda, úsalo aquí:
        // MaterialService.searchMateriales({ nombre: text, descripcion: text })
        //     .then(response => setMateriales(response.data))
        //     .catch(error => console.error("Error al buscar materiales:", error));

        // Si no, filtra localmente:
    ...
    //ELIMINAR
    const deleteMateriales = (id, nombre) => {
        if (window.confirm(`¿Estás seguro de eliminar el material "${nombre}"?`)) {
            MaterialService.deleteMateriales(id).then((response) => {
                listarMateriales();
                alert("Materiales eliminada correctamente");
            }).catch(error => {
                console.log(error);
    ...
    return (
        <div className='app-content'>
            <div class="app-content-header">
                <h1 class="app-content-headerText">Materiales</h1>
            ...
            <div class="app-content-actions">
                ...
                <div class="app-content-actions-wrapper">
                    <Link to="/add-materiales"><button class="app-content-headerButton">Agregar Material</button></Link>
            ...
            <div class="products-area-wrapper tableView">
                ...
                {current.map(material => (
                    <div key={material.id} className="products-row">
                        ...
                        <div className="product-cell image">
                            <img 
                                src={`http://localhost:8060/Imagenes-contructora/${material.imagen}`} 
                                alt={material.nombre} 
                            />
                        </div>

                        <div className="product-cell category"><span className="cell-label">Nombre:</span> {material.nombre}</div>
                        <div className="product-cell sales"><span className="cell-label">Descripcion:</span> {material.descripcion}</div>
                        <div className="product-cell stock"><span className="cell-label">Cantidad:</span> {material.cantidad}</div>
                        <div className="product-cell sales">
                            <span className="cell-label">Unidad:</span> {material.unidades?.nombre || "Sin unidad"}
                        </div>
                        <div className="product-cell sales"><span className="cell-label">Costo Unitario:</span> {material.costo_unitario}</div>
                        <div className="product-cell actions">
                            <Link to={`/edit-materiales/${material.id}`}>
                            ...
                            <button className='app-content-headerButton' title='Eliminar' onClick={() => deleteMateriales(material.id, material.nombre)}>
                                <svg class="w-6 h-6 text-gray-800 dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
...
export default ListMaterialesComponent;
