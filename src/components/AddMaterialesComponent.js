...
export const AddMaterialesComponent = () => {
    ...
    useEffect(() => {
        ...
        if (id) {
            MaterialService.getMaterialesById(id).then((response) => {
                ...
                setIdUnidad(material.unidades?.id || "");
                setCosto_unitario(material.costo_unitario);
                // setImagen(material.imagen);
                if (material.imagen) {
                    setPreview(`http://localhost:8060/Imagenes-contructora/${material.imagen}`);
                }
            }).catch(error => {
    ...
    const saveMateriales = (e) => {
        ...
        formData.append('idUnidad', idUnidad); // Enviar ID de unidad como campo plano
        ...
        if (id) {
            MaterialService.updateMateriales(formData).then(() => {
            ...
            MaterialService.createMateriales(formData).then(() => {
    ...
    //     if (id) {
    //         MaterialService.updateMateriales(materiales).then((response) => {
    ...
    //         });
    //     } else {
    //         MaterialService.createMateriales(materiales).then((response) => {
    //             console.log(response);
    //             navigate("/");
    //         }).catch(error => {
    //             console.log(error);
    ...
    const title = id ? 'Actualizar Material' : 'Registro de Material';
                        ...
                        <form onSubmit={saveMateriales}>
                            <div class="user-details">
                                ...
                                <div className='button'>
                                    {/* <button type="submit" onClick={(e) => saveMateriales(e)}  className="btn btn-primary mb-2">{id ? 'Actualizar' : 'Registrar'}</button>
                                    <Link to="/" className='btn btn-danger mb-2'>Cancelar</Link> */}
...
export default AddMaterialesComponent;
